...
public class PallindromeCheckerApp {
    ...
    public static void main(String[] args) {
        ...
        run("UC02/UC03 - Basic Loop", input,
                PallindromeCheckerApp::basicLoop);

        // UC04
        run("UC04 - Character Array", input,
                PallindromeCheckerApp::charArrayMethod);

        // UC05
        run("UC05 - Stack", input,
                PallindromeCheckerApp::stackMethod);

        // UC06
        run("UC06 - Queue + Stack", input,
                PallindromeCheckerApp::queueStackMethod);

        // UC07
        run("UC07 - Deque", input,
                PallindromeCheckerApp::dequeMethod);

        // UC08
        run("UC08 - LinkedList", input,
                PallindromeCheckerApp::linkedListMethod);

        // UC09
        run("UC09 - Recursive", input,
                PallindromeCheckerApp::recursiveMethod);

        // UC10
        run("UC10 - Normalized", input,
                PallindromeCheckerApp::normalizedMethod);
